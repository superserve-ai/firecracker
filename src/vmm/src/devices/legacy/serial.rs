...
/// Output sink for the serial device, with optional rate limiting and a hard
/// per-boot byte cap. The rate limiter bounds bandwidth; the cap bounds the
/// TOTAL a boot can emit (cap == 0 means uncapped, used by the null sink), so
/// a slow steady writer cannot grow the host-side console file without limit.
#[derive(Debug)]
...
impl SerialOut {
    /// Create a new `SerialOut` with the given inner sink and optional rate
    /// limiter. Stdout and file sinks are additionally capped at
    /// CONSOLE_MAX_BYTES per boot; the discarding sink is uncapped (nothing
    /// reaches a disk).
    pub fn new(inner: SerialOutInner, rate_limiter: Option<TokenBucket>) -> Self {
        let cap = match inner {
            SerialOutInner::Sink => 0,
            SerialOutInner::Stdout(_) | SerialOutInner::File(_) => CONSOLE_MAX_BYTES,
        };
        Self {
            inner,
            rate_limiter,
    ...
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        if self.cap != 0 && self.written >= self.cap {
            if !self.capped_logged {
                ...
                warn!(
                    "serial console hit {}-byte cap; dropping further output",
                    self.cap
                );
            }
        ...
        let n = match &mut self.inner {
            SerialOutInner::Stdout(stdout) => stdout.write(&buf[..take])?,
            SerialOutInner::File(file) => file.write(&buf[..take])?,
            SerialOutInner::Sink => take,
        };
...
mod tests {
    ...
    fn test_serial_out_cap() {
        ...
        let mut out = SerialOut::new(SerialOutInner::File(tmp.as_file().try_clone().unwrap()), None);
        ...
        let mut sink = SerialOut::new(SerialOutInner::Sink, None);
